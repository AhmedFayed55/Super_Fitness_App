...
void main() {
  late HomeRepo repo;
  late RecommendationToDayUseCase useCase;
  ...
  setUp(() {
    ...
    repo = MockHomeRepo();
    useCase = RecommendationToDayUseCase(repo);
  });
  ...
  test("success case for recommendationToDayUseCase", () async {
    ...
    when(repo.recommendationToDay()).thenAnswer((_) async => mockResult);

    var result = await useCase.call();

    verify(repo.recommendationToDay()).called(1);
