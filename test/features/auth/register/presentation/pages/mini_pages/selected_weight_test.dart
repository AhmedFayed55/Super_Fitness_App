...
void main() {
  ...
  setUp(() {
    ...
    when(mockViewModel.state).thenReturn(const RegisterState(weight: 150));

    when(mockViewModel.stream).thenAnswer(
      (_) => Stream<RegisterState>.value(const RegisterState(weight: 150)),
    );
  ...
  testWidgets('SelectedWeight widget shows correct initial Weight and texts', (
    ...
    expect(
      find.byKey(const Key(AppKeys.selectedWeightSubtitle)),
      findsOneWidget,
    );
