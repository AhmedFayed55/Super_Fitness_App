...
import 'selected_hight_test.mocks.dart';
...
void main() {
  ...
  setUp(() {
    ...
    when(mockViewModel.state).thenReturn(RegisterState(height: 170));

    when(
      mockViewModel.stream,
    ).thenAnswer((_) => Stream<RegisterState>.value(RegisterState(height: 170)));
  ...
  testWidgets('SelectedHight widget shows correct initial hight and texts', (
    ...
    expect(find.byKey(const Key(AppKeys.selectedHightSubtitle)), findsOneWidget);
