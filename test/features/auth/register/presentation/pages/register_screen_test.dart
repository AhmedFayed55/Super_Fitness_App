...
void main() {
  ...
  setUp(() {
    ...
    when(mockViewModel.state).thenReturn(RegisterState());
    when(
      mockViewModel.stream,
    ).thenAnswer((_) => Stream<RegisterState>.value(RegisterState()));
  ...
  testWidgets('PageView moves to next page when pageController changes', (
    tester,
  ) async {
    ...
    expect(find.byType(SelectedGender), findsOneWidget);
  });
}
