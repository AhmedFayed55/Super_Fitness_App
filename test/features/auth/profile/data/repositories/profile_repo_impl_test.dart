...
import 'package:super_fitness_app/features/auth/profile/data/repositories/profile_repo_impl.dart';
...
void main() {
  ...
  group('ProfileRepoImpl - getUserData', () {
    test('should return ApiSuccessResult<UserDataResponseEntity> when successful', () async {
      final mockUserDto = UserDataResponseDto(id: "1", firstName: "John", email: "john@example.com");
      final mockResponse = LoggedUserDataResponseDto(user: mockUserDto);

      when(remoteDataSource.getUserData()).thenAnswer((_) async => mockResponse);
    ...
    test('should return ApiErrorResult when user is null in response', () async {
    ...
    test('should return ApiErrorResult on exception', () async {
      when(remoteDataSource.getUserData()).thenThrow(Exception('Unexpected error'));
  ...
  group('ProfileRepoImpl - Local Data Methods', () {
    test('getHelpScreenContent returns ApiSuccessResult', () async {
      final mockHelpDto = [
        HelpScreenResponseDto(section: "Help Section")
      ];
      when(localDataSource.getHelpScreenContent()).thenAnswer((
          _) async => mockHelpDto);
      ...
      final data = (result as ApiSuccessResult<List<HelpScreenResponseEntity>>)
          .data;
    ...
    test(
        'getPrivacyAndSecurityScreenContent returns ApiSuccessResult', () async {
      final mockPrivacyDto = [
        PrivacyPolicyResponseDto(section: "Privacy Title")
      ];
      when(localDataSource.getPrivacyAndSecurityScreenContent()).thenAnswer((
          _) async => mockPrivacyDto);
      ...
      final data = (result as ApiSuccessResult<List<PrivacyPolicyEntity>>).data;
      expect(data.length, mockPrivacyDto.length);
      expect(data.first.section, equals(mockPrivacyDto.first.section));
    });

    test(
        'getSecurityRolesConfigScreenContent returns ApiSuccessResult', () async {
      final mockSecurityDto = [
        SecurityRolesConfigResponseDto(section: "Security Roles")
      ];

      when(localDataSource.getSecurityRolesConfigScreenContent())
      ...
      final data = (result as ApiSuccessResult<List<SecurityRolesConfigEntity>>)
          .data;
